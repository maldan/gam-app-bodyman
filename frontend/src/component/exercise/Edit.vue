...
export default defineComponent({
  ...
  async mounted() {
    const exercise = await RestApi.exercise.get(this.id + '');

    this.name = exercise.name;
    this.tool = { label: exercise.tool, value: exercise.tool };
    this.muscleList = exercise.muscleList?.join(', ');
  },
  methods: {
    async submit() {
      await RestApi.exercise.update(
        this.id + '',
        this.name,
        this.tool.value,
        this.muscleList.split(', '),
      );
      this.$emit('close');
    },
    currentTime() {
