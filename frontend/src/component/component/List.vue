    ...
    <div class="body" style="height: calc(100% - 22px - 15px)">
      ...
      >
        <div class="product_header">
          ...
          <img
            @click="(isShowEditForm = true), (componentId = x.id)"
            class="clickable"
      ...
      <Edit :id="componentId" v-if="isShowEditForm" @close="(isShowEditForm = false), refresh()" />
    </div>
  </div>
</template>
...
export default defineComponent({
  ...
  methods: {
    ...
    async refresh() {
      this.componentList = await RestApi.component.getList();
    },
    async remove(id: string) {
      if (confirm('Are you sure?')) {
        if (confirm('Are you really really sure?')) {
          await RestApi.component.delete(id);
        }
      }
      this.refresh();
    },
